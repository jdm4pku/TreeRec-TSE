...
def train_word2vec(corpus: List[str], vector_size=200, window=5, min_count=1, workers=4):
    """训练 Word2Vec 模型"""
...
def get_avg_vector(text: str, model: Word2Vec) -> np.ndarray:
    """计算平均词向量"""
...
def compute_similarity_word2vec(intents: List[str], candidate_docs: List[str], model: Word2Vec) -> Tuple[List[List[int]], List[float]]:
    """基于 Word2Vec 平均词向量计算相似度"""
...
def run_word2vec_recommendation(data_dir: str, ecosystem: str, output_dir: str,
                                use_fields: List[str], vector_size: int,
                                p_ks: List[int], dcg_ks: List[int]):
    start_time = time.time()
    print(f"⏳ Starting Word2Vec recommendation for [{ecosystem}]...")
    ...
    # 训练 Word2Vec 模型（在候选文档 + 意图上）
    model_corpus = candidate_docs + intents
    model = train_word2vec(model_corpus, vector_size=vector_size)

    # 计算相似度
    top_indices_per_query, query_times = compute_similarity_word2vec(intents, candidate_docs, model)
    ...
    # 保存结果
    output_dir = os.path.join(output_dir, "Word2Vec")
...
def get_parser():
    ...
    parser.add_argument("--vector_size", type=int, default=200, help="Word2Vec embedding size")
