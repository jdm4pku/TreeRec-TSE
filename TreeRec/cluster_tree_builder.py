...
class ClusterTreeBuilder(TreeBuilder):
    ...
    def construct_tree(
        ...
        def process_cluster(
            ...
            logging.info(
                f"Node Texts Length: {len(self.tokenizer.encode(node_texts, disallowed_special=()))}, Summarized Text Length: {len(self.tokenizer.encode(summarized_text, disallowed_special=()))}"
            )

            # For internal nodes, store summary in desc; leave name empty
