...
def get_text(node_list: List[Node]) -> str:
    ...
    for node in node_list:
        # concatenate name and description into a single textual representation
        name_part = (node.name or "").strip()
        desc_part = (node.desc or "").strip()
        combined = name_part if desc_part == "" else (name_part + " : " + desc_part) if name_part != "" else desc_part
        text += " ".join(combined.splitlines())
        text += "\n\n"
    return text
