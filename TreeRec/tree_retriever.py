...
from .EmbeddingModels import (BaseEmbeddingModel, 
                            OpenAIEmbeddingModel, QwenEmbeddingModel, SentenceTransformersEmbeddingModel)
...
class TreeRetrieverConfig:
    def __init__(
        ...
        if context_embedding_model.startswith("Qwen"):
            context_embedding_model = QwenEmbeddingModel()
        elif context_embedding_model.startswith("sentence-transformers"):
            context_embedding_model = SentenceTransformersEmbeddingModel(context_embedding_model)
        else:
            context_embedding_model = OpenAIEmbeddingModel()
        ...
        if embedding_model.startswith("Qwen"):
            ...
            embedding_model = SentenceTransformersEmbeddingModel(embedding_model)
        else:
...
class TreeRetriever(BaseRetriever):
    ...
    def retrieve_information_collapse_tree(self, query: str, top_k: int, max_tokens: int) -> str:
        ...
        for idx in indices[:top_k]:
            ...
            node = node_list[idx]
            # token count on combined name+desc
            ...
            node_tokens = len(self.tokenizer.encode(node_text, disallowed_special=()))
    ...
    def retrieve(
        ...
        if return_layer_information:
            ...
            return selected_nodes, context, layer_information
